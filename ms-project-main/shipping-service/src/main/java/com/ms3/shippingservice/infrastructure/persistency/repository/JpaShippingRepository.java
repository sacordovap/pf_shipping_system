...
public interface JpaShippingRepository extends JpaRepository<ShippingEntity, UUID> {
    ...
    @Query(value = "SELECT * FROM shippings s WHERE (s.origin_branch = :branch OR s.destination_branch = :branch) AND s.current_state = :state", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM shippings s WHERE LOWER(s.tracking_number) LIKE LOWER(CONCAT('%', :term, '%'))", nativeQuery = true)
    List<ShippingEntity> searchByTrackingPartial(@Param("term") String term);

    // Filtro combinado por categoria y estado actual
    @Query("SELECT DISTINCT s FROM ShippingEntity s JOIN s.categories c WHERE c.name = :categoryName AND s.currentState = :state")
    ...
    @Query("SELECT s FROM ShippingEntity s " +
            ...
            "OR LOWER(s.destinatario) LIKE CONCAT('%', LOWER(:name), '%')")
